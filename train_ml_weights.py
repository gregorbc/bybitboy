...
Uso recomendado para clasificador:
  python train_ml_weights.py --type classifier --model randomforest --horizon 4 --up_thr 0.5 --down_thr 0.5 --c_reg 0.1 --candles 40000
"""
...
def train_classifier(symbol, horizon, up_thr, down_thr, c_reg, max_candles, model_type):
    ...
    if model_type == 'randomforest':
        print("🌲 Usando RandomForestClassifier (n_estimators=150, max_depth=8, class_weight='balanced')")
    ...
    if model_type == 'randomforest':
        ...
        for i, feat in enumerate(feature_cols):
            weights[feat] = {
                'DOWN': importances[i] / 3,
                'SIDEWAYS': importances[i] / 3,
                'UP': importances[i] / 3
            }
        classes = ['DOWN', 'SIDEWAYS', 'UP']
        intercepts = [0.0, 0.0, 0.0]
        print("⚠ Nota: RandomForest no produce coeficientes lineales. Se usan importancias normalizadas como pesos.")
    else:
    ...
    print("✅ Clasificador guardado en ml_weights_v2.json")
    return output
