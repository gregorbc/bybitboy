<?php
/**
 * index.php v15.0 – Dashboard ETH/USD Grid Bot (XM MT5)
 * Adaptado para símbolo ETHUSD.
 * Reparado: reemplazadas etiquetas cortas <?= por <?php echo para máxima compatibilidad.
 */
...
$CAPITAL  = (int)($cfg['bot']['capital_usd']     ?? 30);
...
if (isset($_GET['export_pnl'])) {
    ...
    header('Content-Disposition: attachment; filename="pnl_diario_ethusd_' . date('Y-m-d') . '.csv"');
    ...
    if (!empty($mc['host'])) {
        try {
            ...
            $rows = $db->query("SELECT DATE(filled_at) AS fecha, COUNT(*) AS ops,
                ...
                FROM grid_orders WHERE symbol='ETHUSD' AND grid_role='EXIT' AND status='FILLED' AND filled_at IS NOT NULL
...
if (!empty($mc['host'])) {
    try {
        ...
        $db->exec("SET time_zone = '+00:00'");
        $row   = $db->query("SELECT * FROM grid_configs WHERE symbol='ETHUSD' ORDER BY id DESC LIMIT 1")->fetch() ?: [];
        $pnlT  = $db->query("SELECT COALESCE(SUM(pnl_usd),0) p FROM grid_orders WHERE symbol='ETHUSD' AND grid_role='EXIT' AND status='FILLED' AND DATE(filled_at)=CURDATE()")->fetch();
        $fills = $db->query("SELECT COUNT(*) c FROM grid_orders WHERE symbol='ETHUSD' AND grid_role='EXIT' AND status='FILLED'")->fetch();
        $openO = (int)$db->query("SELECT COUNT(*) FROM grid_orders WHERE symbol='ETHUSD' AND status='OPEN'")->fetchColumn();
        ...
        $init  = ['pnl_today'=>(float)($pnlT['p']??0), 'fills_total'=>(int)($fills['c']??0), 'open_orders'=>$openO,
            ...
            'ai_reason'=>preg_replace('/[\x00-\x1F\x7F]/u','',substr($row['ai_reason']??'Evaluando...',0,200))', 'levels'=>(int)($row['levels']??8),
...
<title>ETH/USD · Grid Bot XM · Tiempo Real</title>
...
<div class="app">
  <nav class="topbar">
    ...
    <div class="brand">
      ...
      <div>
        <div class="brand-name">ETH/USD GRID XM</div>
        <div class="brand-sub">MT5 · <?php echo $LEVERAGE; ?>× · <?php echo $CAPITAL; ?> USD · v15.0</div>
      </div>
    </div>
  ...
  <div class="main-grid">
    <div class="sidebar-left" id="sidebarLeft">
      ...
      <div class="card">
        ...
        <div class="cfg-grid">
          <span class="cfg-k">Par</span><span class="cfg-v">ETHUSD</span>
          <span class="cfg-k">Capital</span><span class="cfg-v"><?php echo $CAPITAL; ?> USD</span>
          <span class="cfg-k">Leverage</span><span class="cfg-v"><?php echo $LEVERAGE; ?>×</span>
      ...
      <div class="tab-panels">
        ...
        <div class="tab-panel" id="tab-positions">
          <div class="pos-table-wrap">
            <div class="tbl-wrap">
              <table>
                <thead><tr><th>Lado</th><th>Qty</th><th>Entry $</th><th>uPnL</th><th>Liq $</th></tr></thead>
                <tbody id="posBody"><tr><td colspan="5" class="no-data">Sin posición abierta</td></tr></tbody>
              </table>
            </div>
          </div>
        </div>
        <div class="tab-panel" id="tab-fills">
          ...
          <div class="tbl-wrap">
            <table>
              <thead><tr><th>Hora</th><th>Lado</th><th>Rol</th><th class="tr">PnL</th><th>Price</th><th>R</th></tr></thead>
              <tbody id="fillBody"><tr><td colspan="6" class="no-data">Sin historial</td></tr></tbody>
            </table>
          </div>
          <div class="fills-pg">
        ...
        <div class="tab-panel" id="tab-ml">
          <div class="stat-section">
            ...
            <div class="stat-grid">
              <div class="stat-cell"><div class="stat-lbl">Precisión (OOS)</div><div class="stat-val c-neu" id="mlAccStat">--%</div></div>
              <div class="stat-cell"><div class="stat-lbl">Features</div><div class="stat-val" id="mlFeatCount">--</div></div>
              <div class="stat-cell"><div class="stat-lbl">Símbolo</div><div class="stat-val">ETHUSD</div></div>
...
const AI_INT = <?php echo $AI_INT; ?>;
const CAPITAL_CFG = <?php echo $CAPITAL; ?>;
let SPEED = 'fast';
...
setInterval(()=>{const s=Math.floor((Date.now()-lastStatUpdate)/1000);$('lastUpdate').textContent=s<=0?'ahora':`hace ${s}s`;if(s>8)$('liveIndicator').classList.add('stale');},1000);

function connectWebSocket() {
    const token = '<?php echo EXPORT_TOKEN; ?>';
    const protocol = location.protocol === 'https:' ? 'wss:' : 'ws:';
    const wsUrl = `${protocol}//${location.hostname}:8082?token=${token}`;
    ws = new WebSocket(wsUrl);
    ws.onopen = () => { console.log('[WS] Conectado'); if (wsReconnectTimer) clearTimeout(wsReconnectTimer); const ind = $('wsIndicator'); if (ind) ind.style.background = 'var(--green)'; };
    ws.onmessage = (e) => { try { const data = JSON.parse(e.data); if (!wsInitialDataReceived) { wsInitialDataReceived = true; } updateUIFromWebSocket(data); } catch (err) { console.warn('[WS] parse error', err); } };
    ws.onerror = (err) => { console.warn('[WS] No disponible - usando AJAX polling'); const ind = $('wsIndicator'); if (ind) ind.style.background = 'var(--muted)'; };
    ws.onclose = () => { console.log('[WS] Desconectado, reconectando en 3s...'); const ind = $('wsIndicator'); if (ind) ind.style.background = 'var(--muted)'; wsReconnectTimer = setTimeout(connectWebSocket, 15000); // retry cada 15s si no hay WS };
}
function updateUIFromWebSocket(data) {
    ...
    if (data.uptime) { $('kUpt').textContent = data.uptime; $('uptTxt').textContent = data.uptime; }
    if (data.mode) { $('modeBadge').textContent = data.mode; $('modeBadge').className = `mode-badge m-${data.mode}`; }
    if (data.pair) {
...
async function fetchTicker(){ const d=await fetchWithRetry('_ticker=1','ticker'); if(d&&d.ok) updateTickerUI(d); }
async function fetchStatus(){
  ...
  const pair=d.pairs?.ETHUSD;
  if(pair){
    updatePairUI(pair);
    updatePairNumbers(pair);
    if(pair.orders && pair.orders.length) updateLadder(pair.orders);
  ...
  // Actualizar PnL y fills desde _status
  if(pair){
    if($('kPnlH')&&pair.pnl_today!==undefined) $('kPnlH').innerHTML=fM(pair.pnl_today);
    if($('kPnlHP')&&pair.pnl_today!==undefined) $('kPnlHP').textContent=(pair.pnl_today/CAPITAL*100).toFixed(2)+'% capital';
    if($('kFillsT')&&pair.fills_total!==undefined) $('kFillsT').textContent=pair.fills_total+' fills';
    if($('stFills')&&pair.fills_total!==undefined) $('stFills').textContent=pair.fills_total;
    if($('kOpenO')&&d.open_orders!==undefined) $('kOpenO').textContent=d.open_orders+' órd. abiertas';
    if($('stOpen')&&d.open_orders!==undefined) $('stOpen').textContent=d.open_orders;
    if($('kPrice')&&pair.price) { $('priceLive').textContent='$'+pair.price.toLocaleString('en-US',{minimumFractionDigits:2}); lastPrice=pair.price; }
    if($('cMlAcc')&&pair.ml_accuracy) $('cMlAcc').textContent=(pair.ml_accuracy*100).toFixed(1)+'%';
    if($('mlBadge')&&pair.ml_accuracy) $('mlBadge').textContent='ML '+(pair.ml_accuracy*100).toFixed(0)+'%';
    if(d.real_balance&&$('balVal')) $('balVal').textContent='$'+parseFloat(d.real_balance).toFixed(2);
  }
}
async function fetchLogs(){ const d=await fetchWithRetry('_logs=1','logs'); if(d&&d.lines) appendLogs(d.lines); }
function initLwChart() {
    ...
    const create = () => {
        ...
        lwChart = LightweightCharts.createChart(el, {
            ...
            timeScale: { borderColor: 'rgba(26,37,53,.4)', timeVisible: true, secondsVisible: false, tickMarkFormatter: (t) => { const d = new Date(t * 1000); return d.getUTCHours().toString().padStart(2,'0') + ':' + d.getUTCMinutes().toString().padStart(2,'0'); } }
        });
        ...
        const resizeObserver = new ResizeObserver(() => { if (el.clientWidth > 0 && lwChart) lwChart.applyOptions({ width: el.clientWidth }); });
        resizeObserver.observe(el);
        return true;
    };
    if (!create()) {
        const observer = new IntersectionObserver((entries) => { if (entries[0].isIntersecting && el.clientWidth > 0) { create(); observer.disconnect(); } }, { threshold: 0.1 });
        observer.observe(el);
    }
}
async function fetchMarket(){
  ...
  if(klines.length){
    ...
    if(ohlc.length){
      initLwChart();
      if(lwSeries){ try{lwSeries.setData(ohlc);}catch(e){console.warn('[LW] setData:',e.message);} lastCandleTime=ohlc[ohlc.length-1].time; }
...
async function fetchUpnl(){
  ...
  if(pb){
    if(hasPos){
      pb.innerHTML=d.positions.map(p=>{const amt=parseFloat(p.positionAmt);const side=amt>0?'BUY':'SELL';return `<tr><td><span class="badge ${amt>0?'b-buy':'b-sell'}">${side}</span></td><td>${Math.abs(amt).toFixed(4)}</td><td>${fP(p.entryPrice)}</td><td>${fM(p.unRealizedProfit||0)}</td><td style="color:var(--red)">${fP(p.liquidationPrice)}</td></tr>`;}).join('');
    } else pb.innerHTML='<tr><td colspan="5" class="no-data">Sin posición abierta</td></tr>';
  }
}
async function fetchMLInfo(){
  ...
  barDiv.innerHTML=keys.map(k=>{const v=imps[k]; const pct=maxVal>0?v/maxVal*100:0; return `<div class="ml-feat-row"><span class="ml-feat-name">${k}</span><div class="ml-feat-bar-bg"><div class="ml-feat-bar" style="width:${pct.toFixed(1)}%"></div></div><span class="ml-feat-val">${v.toFixed(3)}</span></div>`;}).join('');
}
...
function startPolling(){ const iv=IV[SPEED]; tickerTimer=setInterval(fetchTicker,iv.tick); statusTimer=setInterval(fetchStatus,iv.stat); logTimer=setInterval(fetchLogs,iv.log); mktTimer=setInterval(fetchMarket,iv.mkt); upnlTimer=setInterval(fetchUpnl,iv.upnl); scalpTimer=setInterval(fetchScalp,iv.scalp); }
function setBotStatus(r){ $('sysTxt').textContent=r?'Activo':'Detenido'; $('sysTxt').style.color=r?'var(--green)':'var(--red)'; $('liveIndicator').className='dot'+(r?' active':''); }
function updateTickerUI(d){
  ...
  if(lwSeries && d.price && lastCandleTime > 0){
    try{ const t5 = Math.floor(Date.now()/1000/300)*300; const ut  = Math.max(t5, lastCandleTime); lwSeries.update({time: ut, open: d.price, high: d.price, low: d.price, close: d.price}); }catch(e){}
  }
}
...
function exportPnl(){window.open('?export_pnl=1&token=<?php echo EXPORT_TOKEN; ?>','_blank');}
...
<?php if($init): ?>
(function(){
  const i=<?php echo json_encode($init, JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_AMP|JSON_UNESCAPED_UNICODE); ?>;
...
connectWebSocket();
startPolling();
fetchTicker(); fetchStatus(); fetchMarket(); fetchUpnl(); fetchScalp(); loadFillsHistory();
