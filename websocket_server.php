<?php
/**
 * websocket_server.php – Grid Bot Dashboard WebSocket Server v3.0
...
$statusFile = dirname($cfgFile) . '/grid_status.json';
$pidFile    = dirname($cfgFile) . '/grid_bot.pid';
$confHist   = dirname($cfgFile) . '/grid_confidence.json';
$dbConfig   = $cfg['mysql'] ?? [];
...
class GridWebSocket implements MessageComponentInterface {
    ...
    public function __construct($dbConfig, $wsToken, $bybitKey, $bybitSecret, $bybitBase) {
    ...
    public function setLoop($loop) {
        ...
        $loop->addPeriodicTimer(1, function () {
            ...
            foreach ($this->clients as $client) $client->send($json);
        });
    }
    ...
    private function collectData(): array {
        ...
        return [
            ...
            'recent_fills'       => $this->getRecentFills(),
    ...
    private function getTicker() {
        global $symbol;
        $r = bybitPub('/v5/market/tickers', ['category' => 'linear', 'symbol' => $symbol]);
        if ($r && !empty($r['list'])) {
            $t = $r['list'][0];
            return [
                'price'       => (float)($t['lastPrice'] ?? 0),
                'change_pct'  => round((float)($t['price24hPcnt'] ?? 0) * 100, 2),
                'high24h'     => (float)($t['highPrice24h'] ?? 0),
                'low24h'      => (float)($t['lowPrice24h'] ?? 0),
                'volume24h'   => (float)($t['volume24h'] ?? 0),
    ...
    private function getStatus() {
        global $statusFile, $pidFile, $logFile;
        ...
        if (file_exists($statusFile)) {
            ...
            if ($st) {
                ...
                $result['pair'] = [
                    ...
                    'levels'          => (int)($pj['levels'] ?? 25),
                    'long_levels'     => (int)($pj['long_levels'] ?? 12),
                    'short_levels'    => (int)($pj['short_levels'] ?? 13),
                    'spacing_pct'     => (float)($pj['spacing_pct'] ?? 0.001),
        ...
        if ($db) {
            try {
                ...
                $result['win_rate']            = ($wr && (int)$wr['t'] > 0) ? round($wr['w'] / $wr['t'] * 100, 1) : null;
    ...
    private function getRecentFills() {
        $db = getDB($this->dbConfig);
        if (!$db) return [];
        try {
            $stmt = $db->prepare("SELECT symbol, side, grid_role, price, qty, pnl_usd, filled_at, is_recovery FROM grid_orders WHERE status='FILLED' ORDER BY filled_at DESC LIMIT 50");
            $stmt->execute();
            return $stmt->fetchAll();
        } catch (Exception $e) { return []; }
    }
    ...
    private function getConfHistory() {
        ...
        if (file_exists($confHist)) {
            $data = json_decode(file_get_contents($confHist), true);
...
$ws = new GridWebSocket($dbConfig, $wsToken, $bybitKey, $bybitSecret, $bybitBase);
$server = IoServer::factory(new HttpServer(new WsServer($ws)), 8082);
echo "=== Grid Bot WebSocket Server v3.0 ===\nEscuchando en puerto 8082\n";
