<?php
/**
 * trainer.php v2.0 – ML Training Center – Grid Bot v13.7
 * Entrena ml_weights_v2.json directamente desde el navegador.
 * – Ejecuta train_ml_weights.py con argumentos CLI
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'start') {
        ...
        $horizon  = max(1, min(24, (int)($_POST['horizon'] ?? 4)));       // 4 velas = 20min
        $up_thr   = max(0.1, min(2.0, (float)($_POST['up_thr'] ?? 0.5))); // 0.5%
        $down_thr = max(0.1, min(2.0, (float)($_POST['down_thr'] ?? 0.5))); // 0.5%
        $candles  = max(5000, min(60000, (int)($_POST['candles'] ?? 40000))); // 40000 velas
        $c_reg    = max(0.01, min(5.0, (float)($_POST['c_reg'] ?? 0.1)));     // regularización 0.1
        $model    = $_POST['model'] ?? 'logistic';
        if (!in_array($model, ['logistic', 'randomforest'])) $model = 'logistic';

        // Construir comando con argumentos CLI
        $cmd = "cd " . escapeshellarg(PUBHTML) .
               " && " . escapeshellarg($py3) . " " . escapeshellarg(TRAIN_PY) .
               " --type classifier" .
        ...
        if ($pid && is_numeric($pid)) {
            ...
            array_unshift($hist, ['ts' => date('Y-m-d H:i:s'), 'pid' => (int)$pid,
                'params' => compact('symbol','horizon','up_thr','down_thr','candles','c_reg','model'), 'status' => 'running']);
...
<title>ML Trainer · Grid Bot v13.7</title>
...
<div class="topbar">
  <div class="brand">
    ...
    <div>
      ...
      <div class="brand-sub">Grid Bot v13.7 · ETH/USDT</div>
    </div>
  </div>
  <div class="tb-right">
...
<div style="display:flex;flex-direction:column;gap:14px">
  <div class="card">
    ...
    <div class="card-bd">
      ...
      <div class="form-row">
        <div class="field"><label>Modelo ML</label><select id="model"><option value="logistic">Regresión Logística</option><option value="randomforest" selected>Random Forest (recomendado)</option></select></div>
        <div class="field"></div>
      </div>
  ...
  <div class="card">
    <div class="card-hd"><h3>📊 Pesos Actuales</h3><?php if ($weights): ?><span style="font-family:var(--mono);font-size:9px;color:var(--muted)"><?= htmlspecialchars($weights['updated_at'] ?? '--') ?></span><?php endif; ?></div>
  ...
  <div class="card">
    ...
    <?php if ($history): ?>
      ...
      <div class="hist-row"><span class="hist-ts"><?= htmlspecialchars($h['ts']) ?></span><span class="hist-tag <?= $h['status']==='running'?'tag-run':'tag-done' ?>"><?= $h['status'] ?></span><span style="color:var(--dim);font-family:var(--mono);font-size:9px"><?= htmlspecialchars($h['params']['symbol']??'') ?> H:<?= $h['params']['horizon']??'' ?> <?= $h['params']['model']??'logistic' ?></span></div>
      <?php endforeach; ?>
    <?php else: ?>
  ...
  <div class="card" id="heatmapCard">
    ...
    <div class="card-bd">
      <?php if ($weights && isset($weights['weights'])): ?>
        ...
        <div style="margin-top:12px"><div class="sec-title">Vectores Scaler</div><div class="grid-2" style="gap:8px;margin-top:6px"><div><div style="font-size:8px;color:var(--muted);margin-bottom:4px">Center (Mediana)</div><?php foreach (($weights['scaler_mean'] ?? []) as $i => $v): ?><div style="display:flex;justify-content:space-between;padding:2px 0;font-family:var(--mono);font-size:9px"><span style="color:var(--muted)"><?= $weights['features'][$i] ?? "feat$i" ?></span><span class="c-blue"><?= number_format($v, 6) ?></span></div><?php endforeach; ?></div><div><div style="font-size:8px;color:var(--muted);margin-bottom:4px">Scale (IQR)</div><?php foreach (($weights['scaler_scale'] ?? []) as $i => $v): ?><div style="display:flex;justify-content:space-between;padding:2px 0;font-family:var(--mono);font-size:9px"><span style="color:var(--muted)"><?= $weights['features'][$i] ?? "feat$i" ?></span><span class="c-purple"><?= number_format($v, 6) ?></span></div><?php endforeach; ?></div></div></div>
      <?php else: ?>
        <div style="text-align:center;padding:30px;color:var(--muted)">Heatmap disponible tras el primer entrenamiento</div>
      <?php endif; ?>
    </div>
  </div>
</div>
...
<script>
const PRESETS = {
  scalping:   {symbol:'ETHUSDT', candles:40000, horizon:4,  up_thr:0.5, down_thr:0.5, c_reg:0.1, model:'randomforest'},
  balanced:   {symbol:'ETHUSDT', candles:30000, horizon:6,  up_thr:0.3, down_thr:0.3, c_reg:0.3, model:'logistic'},
  aggressive: {symbol:'ETHUSDT', candles:25000, horizon:4,  up_thr:0.2, down_thr:0.2, c_reg:0.5, model:'logistic'},
  conservative:{symbol:'ETHUSDT', candles:50000, horizon:12, up_thr:0.8, down_thr:0.8, c_reg:0.05, model:'randomforest'},
};
...
['candles','horizon'].forEach(id => { const el = document.getElementById(id); if(el) el.addEventListener('input', updateHints); });
const term = document.getElementById('term');
function termLine(text, cls='') {
  const last = term.lastElementChild;
  if (last && last.querySelector('.cursor')) last.remove();
  const div = document.createElement('div');
  const tclass = text.includes('✅') || text.includes('Accuracy') ? 't-ok' : (text.includes('❌') || text.includes('Error') ? 't-err' : (text.includes('⚠') ? 't-warn' : (text.includes('📥') || text.includes('📈') ? 't-info' : (text.match(/^(DOWN|SIDEWAYS|UP|MAE|R²)/) ? 't-data' : ''))));
...
const phases = [[0,'Descargando velas de Bybit…'],[20,'Calculando indicadores…'],[40,'Escalado RobustScaler…'],[60,'Entrenando modelo…'],[80,'Evaluando OOS…'],[90,'Exportando ml_weights_v2.json…']];
...
async function startTraining() {
  ...
  const params = { action: 'start', symbol: document.getElementById('symbol').value, candles: document.getElementById('candles').value, horizon: document.getElementById('horizon').value, up_thr: document.getElementById('up_thr').value, down_thr: document.getElementById('down_thr').value, c_reg: document.getElementById('c_reg').value, model: document.getElementById('model').value };
  termLine('', 't-dim'); termLine('════════════════════════════════════════', 't-dim');
  termLine(`🚀 Iniciando entrenamiento: ${params.symbol} | H:${params.horizon} velas | ${params.candles} datos | modelo: ${params.model}`, 't-info');
...
async function pollCompletion() { for (let i=0; i<30; i++) { await new Promise(r=>setTimeout(r,5000)); const d = await fetch('', {method:'POST', body: new URLSearchParams({action:'status'})}).then(r=>r.json()).catch(()=>null); if (!d) break; if (!d.running) { stopPhases(true); document.getElementById('trainBtn').disabled = false; document.getElementById('stopBtn').style.display = 'none'; if (d.weights) updateHeatmap(d.weights); termLine('✅ Entrenamiento completado (detectado via poll).', 't-ok'); break; } } }
