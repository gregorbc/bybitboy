...
<?php
/**
 * ETH/USDT GRID BOT v13.9 – SCALPING ADAPTATIVO MEJORADO
 * 
...
define('G_CAPITAL',       (float)cv($cfg, ['bot', 'capital_usd'], 30.0));
define('G_LEVERAGE',      (int)cv($cfg, ['bot', 'leverage'], 50));
define('G_CYCLE_SEC',     (int)cv($cfg, ['bot', 'cycle_sec'], 8));
define('G_AI_INTERVAL',   (int)cv($cfg, ['bot', 'ai_interval_sec'], 120));
define('G_TF',            (string)cv($cfg, ['bot', 'timeframe'], '5'));
define('G_CANDLES',       (int)cv($cfg, ['bot', 'candles_feed'], 150));
define('G_MIN_LEVELS',    8);
define('G_MAX_LEVELS',    20);
define('G_MIN_SPACING',   (float)cv($cfg, ['bot', 'spacing_min'], 0.0003));
define('G_MAX_SPACING',   (float)cv($cfg, ['bot', 'spacing_max'], 0.0012));
define('G_MARGIN_SAFETY', 0.85);
define('G_MAKER_FEE',     0.0001);
define('G_TAKER_FEE',     0.0006);
define('G_MAX_DAILY_LOSS',12.0);
define('G_HARD_STOP_PCT', 3.0);
...
define('G_FIXED_LEVELS',     (int)cv($cfg, ['bot', 'levels'], 16));
define('G_BASE_SPACING',     (float)cv($cfg, ['bot', 'spacing_base'], 0.0003));
define('G_SPACING_ATR_MULT', (float)cv($cfg, ['bot', 'spacing_atr_mult'], 0.28));
define('G_RECOVERY_LOSS_PCT',3.0);
define('G_MIN_BUILD_INTERVAL',90);
define('G_ML_BLEND_WEIGHT',  0.90);
define('G_ML_RELOAD_CYCLES', 120);
    ...
    public function positions($symbol) {
        ...
        foreach (isset($r['list']) ? $r['list'] : [] as $p) {
            ...
            $out[] = ['positionAmt' => (isset($p['side']) && $p['side'] === 'Buy') ? $sz : -$sz,
                      ...
                      'markPrice' => (float)(isset($p['markPrice']) ? $p['markPrice'] : 0),
                      ...
                      'leverage' => (int)(isset($p['leverage']) ? $p['leverage'] : 1),
                      'positionIM' => (float)(isset($p['positionIM']) ? $p['positionIM'] : 0),
                      'positionMM' => (float)(isset($p['positionMM']) ? $p['positionMM'] : 0),
                      'adlRankIndicator' => (int)(isset($p['adlRankIndicator']) ? $p['adlRankIndicator'] : 0),
                      'createdTime' => (int)(isset($p['createdTime']) ? $p['createdTime'] : 0),
                      'updatedTime' => (int)(isset($p['updatedTime']) ? $p['updatedTime'] : 0)];
        }
        return $out;
    }
...
// ════════════════════════════════════════════════════════
// 12. RISK ENGINE (VaR, Drawdown, Kelly Sizing)
// ════════════════════════════════════════════════════════
class RiskEngine {
    private $db;
...
// ════════════════════════════════════════════════════════
// 13. NOTIFICATION MANAGER (Telegram/Discord Alerts)
// ════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════
// 14. GRID MANAGER (con mejoras de espaciado, persistencia de dirección y notional mínimo)
// ════════════════════════════════════════════════════════
class GridManager {
    private $api;
    private $ai;
    ...
    public function __construct($api, $ai, $ml) {
        ...
        $cfgFile = file_exists(__DIR__ . '/config.json') ? __DIR__ . '/config.json' : dirname(__DIR__) . '/private/config.json';
        $cfg = json_decode(file_get_contents($cfgFile), true) ?: [];
        $this->risk = new RiskEngine(null, $cfg['risk'] ?? []);
        $this->notify = new NotificationManager($cfg['alerts'] ?? []);
    }
    ...
    private function loadVolatilityModel() {
        ...
        $candidates = [$this->volFile, $ridgeFile];
        $chosen = null;
    ...
    private function predictFutureATR($candles) {
        ...
        $featOrder = ['stoch_14', 'macd_hist', 'ema_diff_9_21', 
        ...
        if ($atr_actual > 0.01) {
            ...
            if ($ratio < 0.5) {
                ...
                $pred = 0.65 * $atr_actual + 0.35 * $pred; // blend equilibrado
    ...
    public function run() {
        lI("╔══════════════════════════════════════════╗");
        lI("║  ETH/USDT Grid Bot v13.9 – SCALPING     ║");
        ...
        while ($this->running) {
            ...
            try {
                ...
                // Intervalo adaptativo: alta confianza → eval menos frecuente
        $adaptiveInterval = G_AI_INTERVAL;
        $conf = (int)(isset($this->cfg['confidence']) ? $this->cfg['confidence'] : 50);
        if ($conf >= 85) $adaptiveInterval = G_AI_INTERVAL * 2;      // muy seguro: 240s
        elseif ($conf < 50) $adaptiveInterval = max(60, G_AI_INTERVAL / 2); // inseguro: 60s
        if (time() - $this->lastAI >= $adaptiveInterval) $this->aiEvaluate($price);
                if (!$this->gridBuilt) $this->buildGrid($price);
        // Auto-completar si faltan más de 3 órdenes (pérdida de sync)
    ...
    private function aiEvaluate($price) {
        ...
        $w_heur = 1 - $w_ml;
        // Peso heurístico adaptativo: si ML es SIDEWAYS pero heuristics muestran tendencia fuerte
        $mlIsSideways = ($mlResult['direction'] === 'SIDEWAYS');
        $mlReliable = ($mlAcc >= 0.75);
        $heuristicTrend = abs($hScore) >= 1.0;
        $strongHeuristicTrend = abs($hScore) >= 1.5;
        $trendOverride = false;
        if ($mlIsSideways && ($strongHeuristicTrend || (!$mlReliable && $heuristicTrend))) {
            $trendOverride = true;
            $w_heur = min(0.50, $w_heur * 3);
            $w_ml = 1 - $w_heur;
            lI(sprintf("[AI] Heuristic override: ML=SIDEWAYS(acc=%.0f%%) hScore=%.1f → heur weight=%.0f%%", $mlAcc*100, $hScore, $w_heur*100));
        }
        if ($vlProbs) {
        ...
        // Blend 70/30 (más peso ATR actual medido)
        ...
        $levels = G_FIXED_LEVELS;
        // Skew de niveles según dirección del mercado
        if ($direction === 'UP') {
            $longLev  = (int)round($levels * 0.625); // 10 BUY, 6 SELL — capitalizar alza
            $shortLev = $levels - $longLev;
        } elseif ($direction === 'DOWN') {
            $shortLev = (int)round($levels * 0.625); // 10 SELL, 6 BUY — capitalizar baja
            $longLev  = $levels - $shortLev;
        } else {
            $longLev  = (int)($levels * 0.5);
            $shortLev = $levels - $longLev;
            // SIDEWAYS con sesgo adaptativo: si heuristics muestran tendencia, sesgar 55/45
            if ($heuristicTrend) {
                if ($hScore > 0) {
                    $longLev  = (int)round($levels * 0.55);
                    $shortLev = $levels - $longLev;
                    lI("[BIAS] SIDEWAYS con sesgo alcista: L={$longLev} S={$shortLev}");
                } else {
                    $shortLev = (int)round($levels * 0.55);
                    $longLev  = $levels - $shortLev;
                    lI("[BIAS] SIDEWAYS con sesgo bajista: L={$longLev} S={$shortLev}");
                }
            }
        }
        $f = $this->api->filters(G_SYM);
        ...
        lI(sprintf("[AI] %s conf=%d%% | spacing=%.4f%% | H:%.1f%s | atr_real=%.2f%% atr_pred=%s | niveles=%d | qty=%.4f ETH", 
            $direction, $confidence, $spacing*100, $hScore, ($trendOverride?'[OVERRIDE]':''), $atr_actual, $atr_pred_str, $levels, $qty));
        
        if ($direction !== $prevDir && $this->directionChangeCount == 0) {
            lI("[AI] Dirección $prevDir → $direction → Reconstruyendo grid");
            $this->api->cancelAll(G_SYM);
            usleep(800000); // 0.8s para que cancelAll procese
            // Solo cerrar posiciones que NO tienen EXIT order activa (evitar posición inversa)
            $hasExit = dbx(function($d) {
                return (int)$d->query("SELECT COUNT(*) FROM grid_orders WHERE symbol='" . G_SYM . "' AND status='OPEN' AND grid_role='EXIT'")->fetchColumn();
            });
            if ($hasExit == 0) {
                $this->closeAllPositions(); // seguro: no hay exits pendientes
            } else {
                lI("[AI] Hay $hasExit EXIT orders activas, no cerrando posiciones (dejarlas completar)");
            }
            dbx(function($d) { return $d->prepare("UPDATE grid_orders SET status='CANCELED' WHERE symbol=? AND status='OPEN'")->execute([G_SYM]); });
            $this->gridBuilt = false;
    ...
    private function calcQty($price, $levels, $f, $knownBalance = null) {
        ...
        // Cap: máximo 12% del capital efectivo por nivel (sin límite artificial del 4%)
    ...
    private function buildGrid($price) {
        ...
        $levels   = G_FIXED_LEVELS;
        $spacing  = (float)(isset($cfg['spacing_pct']) ? $cfg['spacing_pct'] : G_BASE_SPACING) * $this->cascadeSpacing;
    ...
    private function onFill($order, $info, $price) {
        // Cascade detection: track rapid fills
        $now = time();
        $this->recentFillTimes[] = $now;
        $this->recentFillTimes = array_filter($this->recentFillTimes, function($t) use ($now) { return $t > $now - 120; });
        $fillsIn2min = count($this->recentFillTimes);
        if ($fillsIn2min >= 5) {
            $this->cascadeSpacing = min(1.8, 1.0 + ($fillsIn2min - 4) * 0.15);
            lW(sprintf("[CASCADE] %d fills en 2min → spacing x%.1f", $fillsIn2min, $this->cascadeSpacing));
        } elseif ($fillsIn2min <= 2) {
            $this->cascadeSpacing = max(1.0, $this->cascadeSpacing * 0.95);
        }
        $cfg = $this->cfg;
        $spacing = (float)(isset($cfg['spacing_pct']) ? $cfg['spacing_pct'] : G_BASE_SPACING) * $this->cascadeSpacing;
        $qty     = (float)$order['qty'];
    ...
    private function riskCheck($price) {
        ...
        foreach ($positions as $pos) {
            ...
            if ($upnl < 0) $totalUnrealizedLoss += abs($upnl);
            $margin = abs(isset($pos['positionAmt']) ? $pos['positionAmt'] : (isset($pos['size']) ? $pos['size'] : 0)) * abs(isset($pos['entryPrice']) ? $pos['entryPrice'] : 0) / G_LEVERAGE;
            $notional = abs(isset($pos['positionAmt']) ? $pos['positionAmt'] : (isset($pos['size']) ? $pos['size'] : 0)) * abs(isset($pos['entryPrice']) ? $pos['entryPrice'] : 0);
            if ($margin > 0 && $upnl < 0 && abs($upnl) / $margin * 100 >= 80) {
                lE(sprintf("[HARD_STOP] uPnL $%.4f (%.0f%% del margen) → cierre forzoso", $upnl, abs($upnl)/$margin*100));
        ...
        if ($totalUnrealizedLoss > 0 && $totalUnrealizedLoss / G_CAPITAL * 100 >= G_HARD_STOP_PCT) {
            lE(sprintf("[HARD_STOP] Pérdida flotante total $%.2f (%.1f%% del capital). Cancelando todo.", $totalUnrealizedLoss, $totalUnrealizedLoss / G_CAPITAL * 100));
            $this->api->cancelAll(G_SYM);
            $this->closeAllPositions();
            $this->gridBuilt = false;
            sleep(600);
    ...
    private function profitOptimize($price) {
        ...
        if ($pct >= G_COMPOUND_THR && !(isset($this->cfg['recovery_active']) && $this->cfg['recovery_active']) && $cooldownOk) {
            ...
            // FIX: multiplicar sobre qty actual, no recalcular desde cero
            $maxAllowed = ($oldQty * 3.0); // máximo 3x el qty inicial
    ...
    private function breakoutCheck($price) {
        ...
        if (!$r || !$r['mn']) return;
        $range = (float)$r['mx'] - (float)$r['mn']; $margin = $range * 0.30; // 30% buffer — menos rebuilds
        if ($price < (float)$r['mn'] - $margin || $price > (float)$r['mx'] + $margin) {
            // No reconstruir si hubo fill en los últimos 90s (precio en movimiento activo)
    ...
    private function writeStatus($price) {
        ...
        $this->savePositionSnapshots($positions);
    ...
    private function savePositionSnapshots(array $positions) {
        if (empty($positions)) return;
        dbx(function($d) use ($positions) {
            $stmt = $d->prepare("INSERT INTO position_snapshots (symbol, side, size, entry_price, mark_price, unrealised_pnl, liquidation_price, leverage, position_im, position_mm, roe_pct, dist_to_liq_pct, mark_to_entry_pct, adl_rank) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            foreach ($positions as $p) {
                $sz  = abs($p['positionAmt'] ?? 0);
                $side = $p['side'] ?? ($p['positionAmt'] > 0 ? 'Buy' : 'Sell');
                $entry = (float)($p['entryPrice'] ?? 0);
                $mark  = (float)($p['markPrice'] ?? 0);
                $upnl  = (float)($p['unRealizedProfit'] ?? 0);
                $liq   = (float)($p['liquidationPrice'] ?? 0);
                $lev   = (int)($p['leverage'] ?? 1);
                $im    = (float)($p['positionIM'] ?? 0);
                $mm    = (float)($p['positionMM'] ?? 0);
                $adl   = (int)($p['adlRank'] ?? 0);
                $roe   = $im > 0 ? round(($upnl / $im) * 100, 4) : 0;
                $dist  = $entry > 0 && $liq > 0 ? round(abs($entry - $liq) / $entry * 100, 4) : 0;
                $me    = $entry > 0 && $mark > 0 ? round(($mark - $entry) / $entry * 100, 4) : 0;
                $stmt->execute(['ETHUSDT', $side, $sz, $entry, $mark, $upnl, $liq, $lev, $im, $mm, $roe, $dist, $me, $adl]);
            }
        });
    }
    ...
    private $trendPositionSide = null;
    private $trendStopPercent = 0.02;
